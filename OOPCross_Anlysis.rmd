...
library(Hmisc)
set.seed(0)
source("./SmallScripts/OOPPlots.r")
...
### Outpatient OOP Rate Plots

```{r, echo=F, fig.height=3, width=7}

png("./Analysis/Figures/OutPatTotal_OOP.png",400,200)
PlotOutPOOP("Total","Outpatient Cost Reported OOP/No-OOP")
dev.off()

png("./Analysis/Figures/OutPatTransport_OOP.png",400,200)
PlotOutPOOP("Transport","Outpatient Transport Reported OOP/No-OOP")
dev.off()

PlotOutPOOP("Medicine","Outpatient Medicine Reported OOP/No-OOP")
PlotOutPOOP("Medical_Fee","Outpatient Medical Fee Reported OOP/No-OOP")

```

### Outpatien OOP Cost Plots
```{r, echo=F,results='asis',fig.height=4,fig.width=4,warning=F}

PlotOutPOOPCost(Var="Total",Inf,"Outpatient Total Costs Reported for OOP")
PlotOutPOOPCost(Var="Total",500000,"Outpatient Total Costs Reported for OOP")
PlotOutPOOPCost(Var="Transport",Inf,"Outpatient Transportation Costs Reported for OOP")
PlotOutPOOPCost(Var="Transport",200000,"Outpatient Transportation Costs Reported for OOP")
PlotOutPOOPCost(Var="Medicine",Inf,"Outpatient Medicine Costs Reported for OOP")
PlotOutPOOPCost(Var="Medical_Fee",Inf,"Outpatient Medical Fee Costs Reported for OOP")


```

### Outpatien OOP Tables

#### Outpatient Total Cost Reports OOP
```{r, echo=F,results='asis'}
...
```{r, echo=F,results='asis',fig.height=6}

NOutPatTrans <- 
  as.data.frame(
      sapply (levels(OutPat$Facility),function(F){
        sapply(OutPatGroups,function(G){ length(subset(OutPat[G,],Facility==F)$Transport) })
      }))
...
#### No-OOP in Outpatient Medical Fee Cost Reports 
```{r, echo=F,results='asis'}

NOutPatMedFee <- 
  as.data.frame(
      sapply (levels(OutPat$Facility),function(F){
        sapply(OutPatGroups,function(G){ length(subset(OutPat[G,],Facility==F)$Medical_Fee) })
      }))
...
NOutPatMedicine <- 
  as.data.frame(
      sapply (levels(OutPat$Facility),function(F){
        sapply(OutPatGroups,function(G){ length(subset(OutPat[G,],Facility==F)$Medicine) })
      }))
...
### Inpatient OOP Rate Plots

```{r, echo=F, fig.height=3, width=7}

png("./Analysis/Figures/InPatTotal_OOP.png",400,200)
PlotInPOOP("Total","Inpatient Cost Reported OOP/No-OOP")
dev.off()

png("./Analysis/Figures/InPatTranport_OOP.png",400,200)
PlotInPOOP("Transport","Inpatient Transport Reported OOP/No-OOP")
dev.off()

PlotInPOOP("Medicine","Inpatient Medicine Reported OOP/No-OOP")
PlotInPOOP("Medical_Fee","Inpatient Medical Fee Reported OOP/No-OOP")

```

### Inpatien OOP Cost Plots
```{r, echo=F,results='asis',fig.height=4,fig.width=4,warning=F}


PlotInPOOPCost(Var="Total",Inf,"Inpatient Total Costs Reported for OOP")
PlotInPOOPCost(Var="Total",5000000,"Inpatient Total Costs Reported for OOP")
PlotInPOOPCost(Var="Transport",Inf,"Inpatient Transportation Costs Reported for OOP")
PlotInPOOPCost(Var="Transport",500000,"Inpatient Transportation Costs Reported for OOP")
PlotInPOOPCost(Var="Medicine",Inf,"Inpatient Medicine Costs Reported for OOP")
PlotInPOOPCost(Var="Medicine",5000000,"Inpatient Medicine Costs Reported for OOP")
PlotInPOOPCost(Var="Medical_Fee",Inf,"Inpatient Medical Fee Costs Reported for OOP")

```

### No-OOP in Inpatient Tables
...
NInPatTrans <- 
  as.data.frame(
      sapply (levels(InPat$Facility),function(F){
        sapply(InPatGroups,function(G){ length(subset(InPat[G,],Facility==F)$Transport) })
      }))
...
NInPatMedFee <- 
  as.data.frame(
      sapply (levels(InPat$Facility),function(F){
        sapply(InPatGroups,function(G){ length(subset(InPat[G,],Facility==F)$Medical_Fee) })
      }))
...
## Detailed analysis of who does/does-not pay

### Who are the no assist who do not pay?

```{r, echo=F}

cat("NoAssist NoOOP OutP Medicine")
NumNoAssisNOOP <- length(subset(OutPat,Group=="NoAssist"&Medicine==0)$Age<5)
NumNoAssisNOOP

cat("NoAssist NoOOP OutP Medicine CU5")
NumNoAssisNOOPCU5 <- sum(subset(OutPat,Group=="NoAssist"&Medicine==0)$Age<5)
NumNoAssisNOOPCU5

paste(format(100*NumNoAssisNOOPCU5/NumNoAssisNOOP,digits=1,scientific=F),"%")


cat("NoAssist NoOOP OutP Medical_Fee")
NumNoAssisNOOP <- length(subset(OutPat,Group=="NoAssist"&Medical_Fee==0)$Age<5)
NumNoAssisNOOP

cat("NoAssist NoOOP OutP Medical_Fee CU5")
NumNoAssisNOOPCU5 <- sum(subset(OutPat,Group=="NoAssist"&Medical_Fee==0)$Age<5)
NumNoAssisNOOPCU5

paste(format(100*NumNoAssisNOOPCU5/NumNoAssisNOOP,digits=1,scientific=F),"%")

cat("NoAssist NoOOP Medicine")
NumNoAssisNOOP <- length(subset(InPat,Group=="NoAssist"&Medicine==0)$Age<5)
NumNoAssisNOOP


a <- with(subset(OutPat,Group=="NoAssist"),table(Facility,Medical_Fee==0,useNA="ifany"))
print(100*a[,2]/rowSums(a),digits=2)

cat("Percent No-Assist No-OOP Medical Fee CU5 by facility")
a <- with(subset(OutPat,Group=="NoAssist"),table(Facility,Medical_Fee==0 & Age<5,useNA="ifany"))
print(100*a[,2]/rowSums(a),digits=2)

OutPat$VillageNum <- factor(lps$Village[OutPat$Serial],levels=Villages$VillageNum)
OutPat$HC <- Villages$Name.HC.coverage[as.numeric(OutPat$VillageNum)]
OutPat$DH <- Villages$Name.DH.coverage[as.numeric(OutPat$VillageNum)]
OutPat$PH <- Villages$Name.PH.coverage[as.numeric(OutPat$VillageNum)]

pat <- subset(OutPat,Group=="NoAssist" & (Facility %in% c("HC","PH","DH")) & Medicine==0 & Age>5)
ins <- lps[as.numeric(as.character(pat$Serial)),grep("HH_Insurance_",names(lps))]
names(ins) <- sub("HH_Insurance_","",names(ins))

table(names(ins)[rowSums(ins,na.rm=T)])/.22


b <- length(pat[,1])

b
cat("Percent No-Assist No-OOP Medicine @ HC by HC Name")
a <- with(subset(OutPat,Medicine==0 & Group=="NoAssist" & Facility=="HC" & Age>5),table(HC))
a
print(100*a/b,digits=2)
print(sum(a/b))
cat("Percent No-Assist No-OOP Medicine @ DH by DH Name")
a <- with(subset(OutPat,Medicine==0 & Group=="NoAssist" & Facility=="DH" & Age>5),table(DH))
a
print(100*a/b,digits=2)
print(sum(a/b))
cat("Percent No-Assist No-OOP Medicine @ PH by PH Name")
a <- with(subset(OutPat,Medicine==0 & Group=="NoAssist" & Facility=="PH" & Age>5),table(PH))
a
print(100*a/b,digits=2)


cat("NoAssist NoOOP Medicine CU5")
NumNoAssisNOOPCU5 <- sum(subset(InPat,Group=="NoAssist"&Medicine==0)$Age<5)
NumNoAssisNOOPCU5

paste(format(100*NumNoAssisNOOPCU5/NumNoAssisNOOP,digits=1,scientific=F),"%")


cat("NoAssist NoOOP Medical_Fee")
NumNoAssisNOOP <- length(subset(InPat,Group=="NoAssist"&Medical_Fee==0)$Age<5)
NumNoAssisNOOP

cat("NoAssist NoOOP Medical_Fee CU5")
NumNoAssisNOOPCU5 <- sum(subset(InPat,Group=="NoAssist"&Medical_Fee==0)$Age<5)
NumNoAssisNOOPCU5
...
`r NumNoAssisNOOP ` have No-OOP `r NumNoAssisNOOPCU5` or are CU5 (`r format(100*NumNoAssisNOOPCU5/NumNoAssisNOOP,digits=1)` %)
...
InPat$GG <- 1:length(InPat[,1])
M <- merge(InPat,lps,"Serial")
length(rowSums(subset(M,Group.x=="NoAssist"&Age>5 & Medicine==0)[,names(lps)[grep("HH_Ins",names(lps))]],na.rm=T))
sum(rowSums(subset(M,Group.x=="NoAssist"&Age>5 & Medicine==0)[,names(lps)[grep("HH_Ins",names(lps))]],na.rm=T)!=8)
...
```{r, echo=F, results='asis', fig.height=3,width=7}
BF <- as.data.frame(with(subset(Mothers,!(BirthFacility %in% c("Other","PC"))),table(BirthFacility,Group,Mother_Birth_Cost_delivery!=0)))
...
BF[!BF$OOP,]$Percent <- 100* BF[!BF$OOP,]$Count/(BF[BF$OOP,]$Count+BF[!BF$OOP,]$Count)
p <- ggplot(data=subset(Mothers,!(BirthFacility %in% c("Other","PC"))))
...
p <- p + geom_text(data=subset(BF,!(BirthFacility %in% c("Other","PC"))),
                   aes(x=BirthFacility,y=Count,
                       label=ifelse(!OOP,
