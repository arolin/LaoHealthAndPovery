...
for (S in 1:maxRow) {
 ...
 IndiHealth=matrix(nrow=indi,ncol=length(QNames)+1);
 ...
 for (S in 1:maxRow ) {
  ...
  for (N in 0:29) {
    ...
    if (!is.na(v) && v) {
      ...
      cdata<-c(cdata,careAt,row["Group"],N+1);
